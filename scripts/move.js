function findMoves(pressedX, pressedY) {
    ...
    if (grid[pressedX][pressedY].type === 'King') {
        options = options.filter(option => {
            return !isBeingAttacked(turnColour, option.x, option.y, pressedX, pressedY)
        })
    } else {
        for (var i = 0; i < grid.length; i++) {
            for (var j = 0; j < grid[i].length; j++) {
                if (!isOpen(i, j) && grid[i][j].type === 'King' && grid[i][j].colorName === turnColour) {
                    options = options.filter(option => {
                        return !isBeingAttacked(turnColour, i, j, pressedX, pressedY, option.x, option.y)
                    })
                    break;
                }
            }
